...
#
	...
	if is_counter_spell:
		...
		if target_class != "Spell" and target_class != "InstantSpell":
			...
			return false
		# 💰 Controllo threshold ApplyToUnderXmanaCost (solo effect_1)
		if data.effect_1_threshold_type == "ApplyToUnderXmanaCost":
			...
			if mana_cost <= threshold_value:
				print("✅ [COUNTER] Enchain consentito: target", target_card.card_data.card_name, "costa", mana_cost, "<=", threshold_value)
				return true
			else:
				print("🚫 [COUNTER] Enchain bloccato: target", target_card.card_data.card_name, "costa", mana_cost, "≥", threshold_value)
				return false

		# Nessun threshold → può sempre enchainare il counter
		return true
