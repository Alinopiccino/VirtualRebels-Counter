...
func on_player_pass_button_pressed(is_manual: bool = false):
	var input_manager = get_parent().get_node_or_null("InputManager")
	if input_manager and input_manager.inputs_disabled and player_action_count == 0:
...
func set_phase(new_phase: Phase):
	...
	if current_phase == Phase.UPKEEP:
		...
		print("🚀 Invio RPC reset_mana dal player:", player_id)
		$"../ManaSlots".rpc("rpc_reset_mana", player_id)
	...
	if current_phase == Phase.MAIN:
		...
		if player_is_attacker:
			...
			attacker_id = multiplayer.get_peers()[0]  # l’altro giocatore
		print("⚔️ [PhaseManager] Assegno l’action all’ATTACCANTE per la fase:", get_phase_name(), "(peer_id =", attacker_id, ")")
		rpc("rpc_give_action", attacker_id)
		rpc_give_action(attacker_id)
...
@rpc("any_peer")
func rpc_give_action(peer_id: int, from_attack: bool = false):
	var my_id = multiplayer.get_unique_id()
	...
	combat_manager.any_combat_in_progress =true
...
#
	...
	match current_phase:
		Phase.START:
			set_phase(Phase.UPKEEP) # 🆕 vai a UPKEEP invece che MAIN
		Phase.UPKEEP:
